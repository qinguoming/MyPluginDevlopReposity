...
def RVE():
    ...
    global vfr,vf0,a,b,a0,b0,r,lmax,lmin,N
    # 生成第一个纤维
    initFiber=FirstFiber()
    fiberList.append(initFiber)
    # fill fiber in windows
    for Fiber_i in fiberList:
        for j in range(N):
            tempFiber=GenerateFiber(Fiber_i)
            # 纤维生成的条件：不自相交 and 在边界之内
            if tempFiber[3]!='out' and IsintersectSelf(tempFiber,fiberList)==False:
                fiberList.append(tempFiber)
            else:
                continue
    # 计算当前的纤维体积分数,满足要求则返回fibers
    t=fiberList
    if CalVolumeFraction(t)>vfr:
        # 开始随机删除纤维
        for i in range(len(t)):
            NowNumOfFibers=len(fiberList)
            # 随机删除其中一个纤维
            del fiberList[random.randint(0, NowNumOfFibers)]
            if abs(CalVolumeFraction(fiberlist=fiberList)-vfr)<0.5*vf0:
                # 生成成功，返回纤维列表
                print('==> Generated successfully, the Volume Fraction is:'+str(CalVolumeFraction(fiberlist=fiberList)))
                return fiberList
...
def ISoverreach(x,y,r):
    """纤维是否与边界线相交,返回值表示相交的位置"""
    global a,b
    
    state=CornerIsInFiber(x,y,r)
...
def GenerateFiber(preFiber):
    ...
    # 判断纤维和边界线的位置关系
    flag=ISoverreach(Center[0],Center[1],r)
    # 返回一个fiber对象
...
def IsintersectSelf(nowfiber,fiberlist):
    """判断当前纤维是否与其他纤维相交"""
    global r
    ...
    if nowfiber[3]=='in':
        ...
        ExpandNowFiber=CalExpendCenter(nowfiber)
...
def CalVolumeFraction(fiberlist):
    global a,b,r
...
def Draw2DXF(fiberlist,dxfFile):
    """将现有的纤维写入到dxf文件"""
    # 创建dxf 文件
    ...
    for fiber in fiberlist:
        ...
        if fiber[3]=='in':
            ...
            # 当纤维和边界相交，特殊处理
            DrawBrokenFiber(fiber=fiber,drawing=drawing)
    drawing.save()
    
def DrawBrokenFiber(fiber,drawing):
    """绘制和边界相交的纤维、位于角点的纤维"""
    global a,b
    r=fiber[2]

    Cs=CalExpendCenter(fiber)
    if len(Cs)==4:
        # 角点在纤维内
...
def ShowInfo():
    global r,a,b,lmax,lmin,vf0,vfr,dxfFile
        # ----- info show -----
    print('\n')
    print(' ----- info show -----')
    ...
    print(' Vf0 = '+str(vf0))
    print(' Low Limit Volume Fraction = '+str(LowLimitFillFraction()))
    print(' Export To DXF = '+str(dxfFile))

def CalExpendCenter(fiber):
...
def DXF2ABAQUS():
    ...
    global fiberLength,a,b,r,model,FiberPartName,dxfFile
    ...
    # 装配
    global FiberInstanceName,MatrixInstanceName
